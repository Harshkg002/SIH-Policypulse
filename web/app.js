...
function renderWordCloud(){
  const text = individualComments.map(c=>c.comment_text).join(" ");
  const stopwords = new Set(['the','and','on','of','is','a','to','in','for','with','be','it','are','this','that','as','an','by','or','from']);
  const words = text.toLowerCase().split(/\W+/).filter(Boolean).filter(w=>!stopwords.has(w));
  const freq = {};
  words.forEach(w=>freq[w]= (freq[w]||0)+1);
  const list = Object.entries(freq).map(([w,c])=>[w,c*10]);
  ...
  WordCloud(wordcloudElement, {
    ...
    color: function(word){
      if(word.includes('penalty')||word.includes('burden')||word.includes('compliance')) return '#ff6b6b';
      return '#94a3b8';
    }
  });
}
